...
export async function runIvm(source: string, filename: string, transport: ScriptingTransport) {
  ...
  vm.setGlobal("printLog", function (message: string) {
    console.log(message)
  })
  vm.run(`
    printLog('From VM!')
  `)
  console.log('Outside VM!')
  ...
  vm.run(
    ...
    global.__handleConnect = function() {
      printLog('__handleConnect')
      for (let handle of __connectEventListeners) {
        handle()
      }
    }

    global.__handleMessage = function(event) {
      printLog('__handleMessage ' + JSON.stringify(event))
      if (global.onmessage) {
    ...
    global.__handleError = function(event) {
      printLog('__handleError ' + JSON.stringify(event))
      if (global.onerror) {
    ...
    global.addEventListener = function(event, handler, opts) {
      printLog('addEventListener ' + JSON.stringify(event))
      if (event == 'message'){
        if (opts?.once === true) {
          __connectEventListeners.push(handler)
        } else {
